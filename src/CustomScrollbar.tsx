...
const CustomScrollbar = ({ height, handleSize, lineCount, currentLine, gotoLine }) => {
  const bar = useRef();
  const handle = useRef();
  ...
  const setNewPosition = (newPosition) => {
    const scaledValue = Math.round((lineCount * newPosition) / (height - handleSize));
  ...
  const startDrag = (e) => {
    ...
    const mouseUpHandler = (e) => {
      ...
      let adjustedPos = newPosition < height - handleSize ? newPosition : height - handleSize;
      if (adjustedPos < 0) {
        adjustedPos = 0;
      }
  ...
  const adjustedPos = position < height - handleSize ? position : height - handleSize;

  const barStyle = { float: "right", width: "0.7em", backgroundColor: "#f0f0f0", height: height + "px" };
  const handleStyle = {
    borderRadius: "0.2em",
    cursor: "pointer",
    position: "relative",
  ...
  return (
    <div ref={bar} style={barStyle} onClick={handleClick} onWheel={handleWheel} onMouseDown={startDrag}>
