...
const GoToLineModal = (props) => {
  ...
  const handleKeys = (e) => {
    ...
    if (e.key === "ArrowUp") {
      const val = gotoSpec[0] === "+" || gotoSpec[0] === "-" ? gotoSpec.substring(1) : gotoSpec;
      if (isNumeric(val)) {
        setGotoSpec("-" + val);
      }
  ...
  return (
    <Modal dimmer="inverted" size="mini" open={show} onClose={() => onClose(null)}>
      ...
      <Modal.Actions>
        ...
        <div className="ui" style={{ padding: "0.75em", textAlign: "left", width: "70%", float: "left" }}>
          TIP: <Icon name="long arrow alternate up" /> / <Icon name="long arrow alternate down" /> - relative jump
        </div>
      </Modal.Actions>
    </Modal>
  );
