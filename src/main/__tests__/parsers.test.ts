...
describe("JSON Parsing", () => {
  it("pareses JSON object and remaps _message and _ts", () => {
    ...
    const parsed = parsers.parseJson(lineObject, { message: "message", timestamp: "timestamp" });
    // console.log(parsed)
  ...
  it("if unable to find mapping for _message renders JSON text", () => {
    ...
    const parsed = parsers.parseJson(lineObject, { message: "message_name", timestamp: "timestamp" });
    // console.log(parsed)
    expect(parsed._message).toBe(lineObject.line);
    expect(parsed.lineNo).toBe(12);
    expect(parsed.timestamp).toBe("2021-03-18T03:59:52.017Z");
...
describe("Text Parsing", () => {
  ...
  it("extract key value pairs", () => {
    ...
    const parsed = parsers.parsePlainText(lineObject, { extractKeyValue: true });
    expect(parsed._message).toBe(lineObject.line);
  ...
  it("extract timestamp", () => {
    ...
    const parsed = parsers.parsePlainText(lineObject, { timestampPattern: /^.{24}/, extractKeyValue: true });
