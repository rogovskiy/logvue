...
type FileOptionsT = { encoding?: FileEncodingT, bufferSize?: number, numCheckpoints?: number, textFormat?: string, jsonOptions?: JsonOptionsT, textOptions?: TextOptionsT };
type JsonOptionsT = { timestamp: string, message: string }
type TextOptionsT = { extractKeyValue: boolean, timestampPattern: string }
type FileStatsT = { lineCount: number, fileSize: number, checkpoints: Array<CheckpointT> };
type CheckpointT = { lineNo: number, offset: number };
type SearchCheckpointT = { searchLineNo: number, lineNo: number, offset: number };
type LineT = { line: string, offset: number, lineNo: number };
type LineOptionsT = { truncated?: boolean }

type ProgressCallbackFn = (progress: number, count: number) => void;
type LineCallbackFn = (line: string, lineNo: number, offsetNumber: number, _options?: LineOptionsT) => boolean|null|undefined;
type SearchT = { query: string };
type FileInfoT = { fileStats: { checkpoints: Array<CheckpointT> }, searchCheckpoints: Map<String, CheckpointT> }
...
const readLinesFromBuffer = (buffer: Buffer, bufferEnd: number, encoding, fileOffset: number, truncatedLine: string|null, startLine: number, lineCallback) => {
    let offset = 0;
    ...
    while (index < bufferEnd) {
        ...
        if (b === 0xD || b === 0xA) {
            ...
            if (line.length > 0) {
                const continueLoop = lineCallback(line, lineCounter++, offset + fileOffset, options);
...
const scanFile = async (filename: string, startingCheckpoint: CheckpointT|null, encoding: FileEncodingT, bufferSize: number, lineCallback: LineCallbackFn) : Promise<void> => {
    ...
    try {
        ...
        do {
            ...
            const [ unusedOffset, lastLineInBuffer, stopped ] = readLinesFromBuffer(buffer, bufferEnd, encoding, fileOffset - bufferOffset, truncatedLine, lastLine, lineCallback);
            if (stopped) {
        ...
        if (bufferOffset > 0 && !iterationStopped) {
            ...
            lineCallback(buffer.toString(encoding, 0, bufferOffset), lastLine, fileOffset - bufferOffset);
        }
    } finally {
...
export const openFile = async (filename: string, lineBufferSize: number, options: FileOptionsT, progressCallback: ProgressCallbackFn|null) : Promise<{lines: Array<LineT>, fileStats: FileStatsT}> => {
    ...
    const checkpointFrequency = totalLength / (numCheckpoints || DEFAULT_CHECKPOINTS);
    const result = { lineCount: 0, fileSize: totalLength, checkpoints: [] };

    await scanFile(filename, null, (encoding || 'utf8'), (bufferSize || DEFAULT_BUFFER), (line, lineNo, offset, _options) => {
...
const locateClosestOffsetToLine = <T>(checkpoints: Array<T>, line, mapper: (T)=>number): T|null => {
    let closest = null;
...
export const loadBuffer = async (filename: string, start: number, lineBufferSize: number, checkpoints: Array<CheckpointT>, options: FileOptionsT, progressCallback: ProgressCallbackFn) : Promise<{ lines: Array<LineT>, offset: number }> => {
    ...
    if (checkpoints) {
        startingCheckpoint = locateClosestOffsetToLine(checkpoints, start, cp => cp.lineNo) || { lineNo: 0, offset: 0};
    }
    const updateFrequency = (start + lineBufferSize - startingCheckpoint.lineNo) / 200;
    ...
    await scanFile(filename, startingCheckpoint, (encoding || 'utf8'), (bufferSize || DEFAULT_BUFFER), (line, lineNo, offset, _options) => {
        if (lineNo >= start) {
        ...
        if (progressCallback) {
            if (lineNo - lastUpdate > updateFrequency) {
                progressCallback(100 * (lineNo - startingCheckpoint.lineNo) / (start + lineBufferSize - startingCheckpoint.lineNo), 0);
                lastUpdate = lineNo;
            }
        }
    });
...
export const searchBuffer = async (filename: string, search: SearchT, startLineNo: number, lineBufferSize: number, checkpoints: Array<SearchCheckpointT>, options: FileOptionsT, progressCallback: ProgressCallbackFn) : Promise<{ lines: Array<LineT> }> => {
    const { encoding, bufferSize, numCheckpoints } = options;
    ...
    if (checkpoints) {
        startingCheckpoint = locateClosestOffsetToLine(checkpoints, startLineNo, cp => cp.searchLineNo) || { lineNo: 0, offset: 0, searchLineNo: 0};
    }
    ...
    await scanFile(filename, startingCheckpoint, (encoding || 'utf8'), (bufferSize || DEFAULT_BUFFER), (line, lineNo, offset, _options) => {
        let lineObject = { line, lineNo, offset, searchLine: startLineNo + lines.length };
        if (lineParser) {
            lineObject = lineParser(lineObject);
        }
...
// returns a buffer of lines starting the given line number
export const searchScan = async (filename: string, search: SearchT, options: FileOptionsT, progressCallback: ProgressCallbackFn) : Promise<{ resultsCount:number, checkpoints: Array<SearchCheckpointT>  }> => {
    const { encoding, bufferSize, numCheckpoints } = options;
